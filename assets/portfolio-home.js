(function () {
  ...
  const apkDownloadSources = {
    github: {
      latestReleaseUrl: "https://api.github.com/repos/ljmng7/YumChicken-Android-Release/releases/latest",
      fallbackUrl: fallbackApkDownloadUrl,
      accept: "application/vnd.github+json"
    },
    gitee: {
      latestReleaseUrl: "https://gitee.com/api/v5/repos/ljmng7/YumChicken-Android-Release/releases/latest",
      fallbackUrl: fallbackGiteeApkDownloadUrl,
      accept: "application/json"
    }
  };
  const latestApkDownloadPromises = {};
  let mainlandChinaDetectionPromise = null;

  const normalizeDownloadUrl = (url, sourceName) => {
    if (typeof url !== "string" || url.trim() === "") {
      return "";
    }

    const normalizedUrl = url.trim();
    if (sourceName === "gitee" && normalizedUrl.startsWith("/")) {
      return `https://gitee.com${normalizedUrl}`;
    }

    return normalizedUrl;
  };

  const getAssetDownloadUrl = (asset, sourceName) =>
    normalizeDownloadUrl(asset.browser_download_url || asset.download_url || "", sourceName);

  const findApkAsset = (assets, sourceName) =>
    assets.find(
      (asset) =>
        typeof asset.name === "string" &&
        asset.name.toLowerCase().endsWith(".apk") &&
        getAssetDownloadUrl(asset, sourceName)
    );

  const fetchGiteeReleaseAttachFiles = async (release) => {
    ...
    const response = await window.fetch(
      `${apkDownloadSources.gitee.latestReleaseUrl.split("/releases/")[0]}/releases/${releaseId}/attach_files`,
      {
        headers: {
          Accept: apkDownloadSources.gitee.accept
        }
      }
    );
    ...
    const attachFiles = await response.json();
    return Array.isArray(attachFiles) ? attachFiles : [];
  };

  const getReleaseAssets = async (release, sourceName) => {
    if (Array.isArray(release.assets)) {
      return release.assets;
    }

    if (Array.isArray(release.attach_files)) {
      return release.attach_files;
    }

    return sourceName === "gitee" ? fetchGiteeReleaseAttachFiles(release) : [];
  };

  const isMainlandChinaCountryValue = (value) =>
    typeof value === "string" &&
    (value.toUpperCase() === "CN" || value.toLowerCase() === "china" || value === "中国");

  const getKnownCountryValues = (data) =>
    [data.country_code, data.countryCode, data.country].filter(
      (value) => typeof value === "string" && value.trim() !== ""
    );

  const fetchLatestApkDownloadUrl = (sourceName, fallbackOnFailure = true) => {
    const source = apkDownloadSources[sourceName];

    if (!source) {
      return Promise.resolve(fallbackOnFailure ? fallbackApkDownloadUrl : null);
    }

    const cacheKey = fallbackOnFailure ? sourceName : `${sourceName}:no-fallback`;

    if (latestApkDownloadPromises[cacheKey]) {
      return latestApkDownloadPromises[cacheKey];
    }

    latestApkDownloadPromises[cacheKey] = (async () => {
      try {
        const response = await window.fetch(source.latestReleaseUrl, {
          headers: {
        ...
        if (!response.ok) {
          return fallbackOnFailure ? source.fallbackUrl : null;
        }

        const release = await response.json();
        const releaseAssets = await getReleaseAssets(release, sourceName);
        const apk = findApkAsset(releaseAssets, sourceName);

        return apk ? getAssetDownloadUrl(apk, sourceName) : fallbackOnFailure ? source.fallbackUrl : null;
      } catch {
        return fallbackOnFailure ? source.fallbackUrl : null;
      }
    })();
    ...
    mainlandChinaDetectionPromise = (async () => {
      try {
        ...
        if (response.ok) {
          ...
          const countryValues = getKnownCountryValues(data);
          if (countryValues.length > 0) {
            return countryValues.some(isMainlandChinaCountryValue);
          }
        }
      } catch {}
      ...
      try {
        const response = await window.fetch("https://api.country.is/");
  ...
  const chooseApkDownloadUrl = async () => {
    ...
    if (giteeMirrorReady && isMainlandChina) {
      return (await fetchLatestApkDownloadUrl("gitee", false)) || fetchLatestApkDownloadUrl("github");
    }

    return fetchLatestApkDownloadUrl("github");
  };
